...
module.exports.up = async function (done) { // eslint-disable-line
  ...
  try {
    const ProceduresModel = mongoose.model('Procedure', ProcedureSchema);
    ...
    const savePromises = Procedures.map(procedure => {
      ...
      procedure.voteResults.partyVotes = procedure.voteResults.partyVotes.map(partyVote => { // eslint-disable-line
